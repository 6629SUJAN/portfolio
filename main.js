...
(function initContactForm() {
  ...
  const serviceId = form.dataset.serviceId || 'YOUR_SERVICE_ID';
  const templateId = form.dataset.templateId || 'YOUR_TEMPLATE_ID';
  const publicKey = form.dataset.publicKey || 'YOUR_PUBLIC_KEY_HERE';
  ...
  if (publicKey && publicKey !== 'YOUR_PUBLIC_KEY_HERE' && typeof emailjs !== 'undefined') {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const message = trimValue(messageInput);

    // Basic beginner-friendly validation before sending the email.
    if (name.length < 2) {
      setMessage('Please enter your name.', 'var(--neon3)');
      nameInput?.focus();
      return;
    }
    if (!isValidEmail(email)) {
      setMessage('Please enter a valid email address.', 'var(--neon3)');
      emailInput?.focus();
      return;
    }
    if (message.length < 10) {
      setMessage('Your message should be at least 10 characters.', 'var(--neon3)');
      messageInput?.focus();
      return;
    }

    if (serviceId === 'YOUR_SERVICE_ID' || templateId === 'YOUR_TEMPLATE_ID' || publicKey === 'YOUR_PUBLIC_KEY_HERE') {
      setMessage('Set up your EmailJS Service ID, Template ID, and Public Key first.', 'var(--neon3)');
      return;
    }

    if (typeof emailjs === 'undefined') {
      setMessage('EmailJS failed to load. Check your internet connection and reload the page.', 'var(--neon3)');
      return;
    }
    ...
    setMessage('Sending your message...', 'var(--neon2)');

    try {
      await emailjs.send(serviceId, templateId, formData);
      form.reset();
      setMessage('Message sent successfully. I will reply soon.', '#22c55e');
    } catch (error) {
      console.error('EmailJS error:', error);
      setMessage('Message could not be sent. Please email me directly at sujann045@gmail.com.', 'var(--neon3)');
    } finally {
